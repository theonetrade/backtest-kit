...
class ReportStorage {
  ...
  public async getReport(
    ...
    return [
      ...
      "",
      "",
      `*Optimization Metric: the name of the per-strategy statistic that this walker run ranked strategies by. The "Best ${results.metric}" value below is the largest value of that statistic among all tested strategies — every selectable walker metric is maximized (higher is always better); the strategy that produced it is reported as Best Strategy. Strategies whose ${results.metric} is N/A are skipped from "best" selection — a missing value never wins.*`,
