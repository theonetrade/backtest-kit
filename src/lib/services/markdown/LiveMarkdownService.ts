...
class ReportStorage {
  ...
  public async getReport(
    ...
    return [
      ...
      "",
      `*Win Rate: percent of closed events that ended with pnl > 0, computed as winCount / (winCount + lossCount) × 100 — break-even closed events (pnl == 0) are excluded from both numerator and denominator. UNITS: percent in [0, 100]. Statistical reliability: noisy below 30 closed events (a single streak can shift it 10–20 points); stable above 200.*`,
      `*Average PNL: arithmetic mean of pnl across every closed event, computed as Σ pnl / totalClosed. UNITS: percent per trade. Sign mirrors strategy edge — positive = profitable on average per trade. Sensitive to one whale trade; cross-check with Median PNL to detect skew.*`,
      `*Total PNL: arithmetic sum of pnl across every closed event (Σ pnl). UNITS: percent. This is the additive total, NOT the compounded equity return — for the geometrically-compounded variant see Recovery Factor's numerator and Expected Yearly Returns. Useful as a quick scoreboard but ignores volatility drag.*`,
      `*Standard Deviation Per Trade: sample standard deviation (Bessel-corrected, N−1 denominator) of per-event pnl. UNITS: percent. Measures volatility of realised returns. Denominator for per-trade Sharpe. Below STDDEV_EPSILON = 1e-9 it is treated as zero (identical-returns guard). Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Sharpe Ratio: per-trade Sharpe = avgPnl / stdDev (risk-free rate = 0). UNITS: dimensionless ratio. Higher = better risk-adjusted return per trade. Rule of thumb: below 1.0 poor, 1.0–2.0 acceptable, above 2.0 strong. Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS} OR stdDev ≤ 1e-9.*`,
      `*Annualized Sharpe Ratio: per-trade Sharpe × √tradesPerYear, where tradesPerYear = totalClosed × 365 / calendarSpanDays. UNITS: dimensionless. Null when totalClosed < ${MIN_SIGNALS_FOR_ANNUALIZATION}, OR span < ${MIN_CALENDAR_SPAN_DAYS} days, OR raw tradesPerYear > ${MAX_TRADES_PER_YEAR}. Assumes returns are iid — autocorrelated strategies are overstated.*`,
      `*Certainty Ratio: avgWin / |avgLoss|, where avgWin = mean pnl over winning closed events and avgLoss = mean over losing closed events. UNITS: dimensionless ratio. Below 1.0 means the typical loss exceeds the typical win; above 1.5 is generally good. Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS}, OR there are no losing events, OR |avgLoss| < 1e-9.*`,
      `*Expected Yearly Returns: geometric annualization of the equity curve: (equityFinal^(tradesPerYear / N) − 1) × 100, where equityFinal is the compounded product of (1 + pnl/100) over all closed events in chronological close order. UNITS: percent per year. Accounts for volatility drag. Null under the same N / span / frequency gates as Annualized Sharpe, AND null when |raw value| > ${MAX_EXPECTED_YEARLY_RETURNS}%. −100% when the equity curve hits ≤ 0 (account blown).*`,
      `*Avg Peak PNL: arithmetic mean of \`event.peakPnl\` (the trade's peak unrealised PNL while open) over closed events that carry that snapshot — events without one are excluded from both numerator and denominator. UNITS: percent. Describes the typical best-case unrealised PNL during the position's open lifetime, not the realised close. NOT gated by MIN_SIGNALS — computed whenever ≥ 1 closed event carries the snapshot.*`,
      `*Avg Max Drawdown PNL: arithmetic mean of \`event.fallPnl\` (the trade's worst unrealised PNL while open) over closed events that carry that snapshot. UNITS: percent (negative for losing excursions). Describes the typical worst-case unrealised PNL during the position's open lifetime; closer to 0 is better. NOT gated by MIN_SIGNALS — computed whenever ≥ 1 closed event carries the snapshot.*`,
      `*Sortino Ratio: avgPnl / downsideDeviation, where downsideDeviation = √( Σ min(0, pnl)² / totalClosed ) (canonical Sortino 1991: MAR = 0, divide by N_total). UNITS: dimensionless. Penalises only downside volatility. Rule of thumb: below 1.0 poor, 1.0–2.0 acceptable, above 2.0 strong. Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS}, OR there are no losing events, OR downsideDeviation ≤ 1e-9 (float-artifact guard).*`,
      `*Calmar Ratio: expectedYearlyReturns / equityMaxDrawdown, clamped to ±${MAX_CALMAR_RATIO}. Denominator (equityMaxDrawdown) is the mark-to-market max drawdown of the compounded equity curve: each closed event's intra-trade trough (\`fallPnl\`, ≤ 0) is applied before booking its realised close, so a position that dipped to −18% and recovered to +2% contributes a real drawdown rather than zero. UNITS: dimensionless. Rule of thumb: below 0.5 poor, 0.5–1.0 acceptable, above 1.0 strong. Null when expectedYearlyReturns is null OR equityMaxDrawdown ≤ 0.*`,
      `*Recovery Factor: (equityFinal − 1) × 100 / equityMaxDrawdown, clamped to ±${MAX_CALMAR_RATIO}. Numerator is the compounded total return (NOT arithmetic Σ pnl — denominator is compounded, mixing units would inflate Recovery on long streaks). Denominator is the mark-to-market max drawdown of the compounded equity curve (same definition as Calmar's denominator). UNITS: dimensionless. Below 1.0 = profit doesn't cover drawdown; above 3.0 = good. Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS}, account blown, or equityMaxDrawdown ≤ 0.*`,
      `*Expectancy: per-trade expected value = (winCount / totalClosed) × avgWin + (lossCount / totalClosed) × avgLoss. Break-even events contribute 0 (excluded from both probabilities). UNITS: percent per trade. Positive = profitable on average per trade. Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Median PNL: median of pnl across all closed events (middle value after sorting; for even N the mean of the two middles). UNITS: percent. Robust to outliers — comparing it with Average PNL reveals distribution skew. A large gap (e.g. avg +1.5%, median +0.2%) means one or two events carry the arithmetic mean. NOT gated by MIN_SIGNALS — computed whenever totalClosed ≥ 1.*`,
      `*Avg Duration: arithmetic mean of (event.timestamp − event.pendingAt) / 60_000 over all closed events. UNITS: minutes. Describes the typical position hold time. Events lacking pendingAt fall back to event.timestamp (zero duration). NOT gated by MIN_SIGNALS — computed whenever totalClosed ≥ 1.*`,
      `*Avg Win Duration: arithmetic mean of (event.timestamp − event.pendingAt) / 60_000 restricted to winning closed events (pnl > 0). UNITS: minutes. Null only when there are no winning events (NOT gated by MIN_SIGNALS). Pair with Avg Loss Duration to detect the classic asymmetry "let winners run, cut losers short" versus the inverse "cut winners short, let losers run".*`,
      `*Avg Loss Duration: arithmetic mean of (event.timestamp − event.pendingAt) / 60_000 restricted to losing closed events (pnl < 0). UNITS: minutes. Null only when there are no losing events (NOT gated by MIN_SIGNALS).*`,
      `*Avg Consecutive Win PNL: a "win streak" is a run of consecutive closed events with pnl > 0 bounded by either a losing or break-even event; this metric sums pnl within each streak and then averages those sums. The walk is over validClosed in chronological close order (reverse-iterate the newest-first storage). UNITS: percent per streak. Higher = winning streaks are typically bigger. Null only when there is not a single complete win streak (NOT gated by MIN_SIGNALS).*`,
      `*Avg Consecutive Loss PNL: a "loss streak" is a run of consecutive closed events with pnl < 0 bounded by either a winning or break-even event; this metric sums pnl within each streak and then averages those sums. UNITS: percent per streak (negative). Closer to 0 = losing streaks are typically smaller. Null only when there is not a single complete loss streak (NOT gated by MIN_SIGNALS).*`,
      `*Trend: classification computed FROM CLOSING PRICES OF CLOSED EVENTS (one (event.timestamp, event.currentPrice) point per closed event; Live ingests sub-minute events but the price profile uses ONLY closed-event prices — no candles, no order book, no tick stream). Bivariate gate: "sideways" when R² < 0.30. Otherwise "neutral" when |trendStrength| < 0.25 × medianStepSize. Otherwise "bullish" if trendStrength > 0, "bearish" if trendStrength < 0. Two-axis gate — never a single-axis if on a magic-constant slope magnitude. Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Trend Strength: ordinary-least-squares slope of \`log(close) ~ days\` regressed across CLOSING PRICES OF CLOSED EVENTS in chronological order. UNITS: percent per day (small-slope approximation). NOT ATR-normalised, NOT ADX, NOT a rolling-window indicator at signal entry — a static fit over the entire stored history at report time. Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS} or the calendar span is degenerate.*`,
      `*Trend Confidence (R²): coefficient of determination of the same log-price regression that produces Trend Strength, computed on log(close) — NOT on absolute close. UNITS: dimensionless in [0, 1]. High R² = log-price moves linearly with time (clean trend); low R² = the series is noisy regardless of slope sign. Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS} or the calendar span is degenerate.*`,
      `*Buyer Pressure: frequency-based, computed FROM CLOSING PRICES OF CLOSED EVENTS. buyerPressure = (count of i where close[i] > close[i−1]) / (count of decisive moves), where "decisive" excludes flats. UNITS: dimensionless fraction in [0, 1]. NOT order-flow aggressor volume — Live's ingest does not separate buy/sell side; "buyer" labels only the SIGN of the close-to-close return. Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Seller Pressure: frequency-based, computed FROM CLOSING PRICES OF CLOSED EVENTS. sellerPressure = (count of i where close[i] < close[i−1]) / (count of decisive moves). By construction sellerPressure = 1 − buyerPressure (Σpressure = 1). UNITS: dimensionless fraction in [0, 1]. NOT order-flow aggressor volume. Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Buyer Strength: magnitude-based, computed FROM CLOSING PRICES OF CLOSED EVENTS. buyerStrength = Σ |close[i] − close[i−1]| / close[i−1] over up-moves, divided by the same sum over ALL decisive moves. UNITS: dimensionless fraction in [0, 1]. Buyer Pressure (count of up-moves) and Buyer Strength (sum of up-magnitudes) use the SAME close-to-close return series; only count vs magnitude differs. A divergence between them (e.g. Pressure 0.70 with Strength 0.45) means "many small up-moves, fewer but larger down-moves". Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Seller Strength: magnitude-based, computed FROM CLOSING PRICES OF CLOSED EVENTS. sellerStrength = Σ |close[i] − close[i−1]| / close[i−1] over down-moves, divided by the same sum over ALL decisive moves. By construction sellerStrength = 1 − buyerStrength (Σstrength = 1). UNITS: dimensionless fraction in [0, 1]. Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Pressure Imbalance: DIFFERENCE, not ratio: buyerStrength − sellerStrength, computed FROM CLOSING PRICES OF CLOSED EVENTS. Equivalent to (2 × buyerStrength − 1). UNITS: dimensionless in [−1, +1]. Sign = direction of magnitude bias (positive = bullish bias on magnitude); absolute value = how lopsided. Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Median Step Size: median over i ∈ [1..N−1] of |close[i] − close[i−1]| / close[i−1], computed FROM CLOSING PRICES OF CLOSED EVENTS — "step" = consecutive closed-event prices (NOT ticks, NOT bars; the report has no candle access even though Live ingests sub-minute events — they are not used here). UNITS: percent, normalised by price → directly comparable across symbols at any price level. Median (not mean) → robust to one whale trade. NOT classical candle-based volatility (ATR / σ of returns); it measures the step distribution AT THE RATE TRADES CLOSE. Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*General reliability note: per-trade ratios (Sharpe, Sortino, Certainty, Recovery, Expectancy) are gated to N/A below ${MIN_SIGNALS_FOR_RATIOS} closed events. Annualised metrics additionally require span ≥ ${MIN_CALENDAR_SPAN_DAYS} days and raw tradesPerYear ≤ ${MAX_TRADES_PER_YEAR}. 100+ closed events are needed for statistical reliability of the ratios.*`,
      `*IMPORTANT: Total PNL and equity-curve metrics (Expected Yearly Returns, Calmar, Recovery, equity-max-drawdown) all assume **100% capital allocation per position** (no portfolio fraction). They ignore the position-sizing subsystem: pnl is a return on the position's own invested capital, never scaled by account balance. With DCA the cost basis is the sum of all entries and the entry price is dollar-cost-weighted. If your strategy risks X% of capital per trade, the realised portfolio return / drawdown is roughly X/100 of the reported figures — these are theoretical upper bounds under full allocation.*`,
