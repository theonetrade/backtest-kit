...
class ReportStorage {
  ...
  public async getReport(
    ...
    return [
      ...
      `*Avg Peak PNL: arithmetic mean of \`event.peakPnl\` (the trade's peak unrealised PNL while open) over closed events that carry that snapshot — events without one are excluded from both numerator and denominator. UNITS: percent. Describes the typical best-case unrealised PNL during the position's open lifetime, not the realised close.*`,
      `*Avg Max Drawdown PNL: arithmetic mean of \`event.fallPnl\` (the trade's worst unrealised PNL while open) over closed events that carry that snapshot. UNITS: percent (negative for losing excursions). Describes the typical worst-case unrealised PNL during the position's open lifetime; closer to 0 is better.*`,
      `*Sortino Ratio: avgPnl / downsideDeviation, where downsideDeviation = √( Σ min(0, pnl)² / totalClosed ) (canonical Sortino 1991: MAR = 0, divide by N_total). UNITS: dimensionless. Penalises only downside volatility. Rule of thumb: below 1.0 poor, 1.0–2.0 acceptable, above 2.0 strong. Null when totalClosed < ${MIN_SIGNALS_FOR_RATIOS}, OR there are no losing events.*`,
      ...
      `*Median PNL: median of pnl across all closed events. UNITS: percent. Robust to outliers — comparing it with Average PNL reveals distribution skew. A large gap (e.g. avg +1.5%, median +0.2%) means one or two events carry the arithmetic mean.*`,
      `*Avg Duration: arithmetic mean of (event.timestamp − event.pendingAt) / 60_000 over all closed events. UNITS: minutes. Describes the typical position hold time. Events lacking pendingAt fall back to event.timestamp (zero duration).*`,
      `*Avg Win Duration: arithmetic mean of (event.timestamp − event.pendingAt) / 60_000 restricted to winning closed events (pnl > 0). UNITS: minutes. Null when there are no winning events. Pair with Avg Loss Duration to detect the classic asymmetry "let winners run, cut losers short" versus the inverse "cut winners short, let losers run".*`,
      `*Avg Loss Duration: arithmetic mean of (event.timestamp − event.pendingAt) / 60_000 restricted to losing closed events (pnl < 0). UNITS: minutes. Null when there are no losing events.*`,
      `*Avg Consecutive Win PNL: a "win streak" is a run of consecutive closed events with pnl > 0 bounded by either a losing or break-even event; this metric sums pnl within each streak and then averages those sums. UNITS: percent per streak. Higher = winning streaks are typically bigger. Null when there is not a single complete win streak in the stored history.*`,
      `*Avg Consecutive Loss PNL: a "loss streak" is a run of consecutive closed events with pnl < 0 bounded by either a winning or break-even event; this metric sums pnl within each streak and then averages those sums. UNITS: percent per streak (negative). Closer to 0 = losing streaks are typically smaller. Null otherwise.*`,
