...
class ReportStorage {
  ...
  public async getReport(
    ...
    return [
      ...
      `*Standard Deviation Per Trade: sample standard deviation (Bessel-corrected, N−1 denominator) of per-trade PNL across closed events. UNITS: percent. Measures volatility of realised returns. Denominator for per-trade Sharpe Ratio. Below STDDEV_EPSILON = 1e-9 it is treated as zero (identical-returns guard). Null when the closed event count < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      ...
      `*Trend: classification computed FROM CLOSING PRICES OF CLOSED EVENTS (one point per closed event = the price at which it closed, with its close-timestamp; the live mode ingests sub-minute events of other kinds but the price profile uses ONLY closed-event prices — no candles, no order book, no tick stream). Bivariate gate: "sideways" when R² < 0.30. Otherwise "neutral" when |Trend Strength| < 0.25 × Median Step Size. Otherwise "bullish" if Trend Strength > 0, "bearish" if Trend Strength < 0. Two-axis gate — never a single-axis if on a magic-constant slope magnitude. Null when the closed event count < ${MIN_SIGNALS_FOR_RATIOS}, which means Median Step Size, Trend Strength and Trend Confidence all gate as one bundle: if any is null the others are null too, so the classification never sees a partially-computed input. Boundary case: when every closed price is identical (a flat synthetic series at N ≥ ${MIN_SIGNALS_FOR_RATIOS}), Median Step Size is 0 and the slope-vs-step gate collapses — the regression then returns slope = R² = 0, R² < 0.30 fires first, and the classification is "sideways" as expected.*`,
      `*Trend Strength: ordinary-least-squares slope of \`log(close) ~ days\` regressed across CLOSING PRICES OF CLOSED EVENTS in chronological order. UNITS: percent per day (small-slope approximation). NOT ATR-normalised, NOT ADX, NOT a rolling-window indicator at signal entry — a static fit over the entire stored history at report time. Null when the closed event count < ${MIN_SIGNALS_FOR_RATIOS} or the calendar span is degenerate.*`,
      `*Trend Confidence (R²): coefficient of determination of the same log-price regression that produces Trend Strength, computed on log(close) — NOT on absolute close. UNITS: dimensionless in [0, 1]. High R² = log-price moves linearly with time (clean trend); low R² = the series is noisy regardless of slope sign. Null when the closed event count < ${MIN_SIGNALS_FOR_RATIOS} or the calendar span is degenerate.*`,
      `*Buyer Pressure: frequency-based, computed FROM CLOSING PRICES OF CLOSED EVENTS. (count of i where close[i] > close[i−1]) / (count of decisive moves), where "decisive" excludes flats. UNITS: dimensionless fraction in [0, 1]. NOT order-flow aggressor volume — the live ingest does not separate buy/sell side; "buyer" labels only the SIGN of the close-to-close return. Null when the closed event count < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Seller Pressure: frequency-based, computed FROM CLOSING PRICES OF CLOSED EVENTS. (count of i where close[i] < close[i−1]) / (count of decisive moves). By construction Seller Pressure = 1 − Buyer Pressure (the two sum to 1). UNITS: dimensionless fraction in [0, 1]. NOT order-flow aggressor volume. Null when the closed event count < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Buyer Strength: magnitude-based, computed FROM CLOSING PRICES OF CLOSED EVENTS. Σ |close[i] − close[i−1]| / close[i−1] over up-moves, divided by the same sum over ALL decisive moves. UNITS: dimensionless fraction in [0, 1]. Buyer Pressure (count of up-moves) and Buyer Strength (sum of up-magnitudes) use the SAME close-to-close return series; only count vs magnitude differs. A divergence between them (e.g. Pressure 0.70 with Strength 0.45) means "many small up-moves, fewer but larger down-moves". Null when the closed event count < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Seller Strength: magnitude-based, computed FROM CLOSING PRICES OF CLOSED EVENTS. Σ |close[i] − close[i−1]| / close[i−1] over down-moves, divided by the same sum over ALL decisive moves. By construction Seller Strength = 1 − Buyer Strength (the two sum to 1). UNITS: dimensionless fraction in [0, 1]. Null when the closed event count < ${MIN_SIGNALS_FOR_RATIOS}.*`,
