...
class ReportStorage {
  ...
  public async getReport(
    ...
    return [
      ...
      `*Avg Max Drawdown PNL: arithmetic mean of \`signal.maxDrawdown.pnlPercentage\` over signals that carry a trough snapshot (excluded otherwise). UNITS: percent (negative for losing excursions). Describes the typical worst-case unrealized PNL during the position's open lifetime; closer to 0 is better.*`,
      `*Sortino Ratio: avgPnl / downsideDeviation, where downsideDeviation = √( Σ min(0, pnlPercentage)² / totalSignals ) (canonical Sortino 1991: MAR = 0, divide by N_total). UNITS: dimensionless. Penalises only downside volatility. Rule of thumb: below 1.0 poor, 1.0–2.0 acceptable, above 2.0 strong. Null when totalSignals < ${MIN_SIGNALS_FOR_RATIOS}, OR there are no losing trades (downside is undefined — flawless ≠ infinitely good).*`,
      ...
      `*Median PNL: median of pnlPercentage across all closed signals (middle value after sorting; for even N the mean of the two middles). UNITS: percent. Robust to outliers — comparing it with Average PNL reveals distribution skew. A large gap (e.g. avg +1.5%, median +0.2%) means one or two trades carry the arithmetic mean.*`,
      `*Avg Duration: arithmetic mean of (closeTimestamp − pendingAt) / 60_000 over all closed signals. UNITS: minutes (synchronised with strategy \`minuteEstimatedTime\`). Describes the typical position hold time.*`,
      `*Avg Win Duration: arithmetic mean of (closeTimestamp − pendingAt) / 60_000 restricted to winning trades (pnlPercentage > 0). UNITS: minutes. Null when there are no winning trades. Pair with Avg Loss Duration to detect the classic asymmetry "let winners run, cut losers short" (Win Duration > Loss Duration is healthy) versus the inverse red flag "cut winners short, let losers run".*`,
      `*Avg Loss Duration: arithmetic mean of (closeTimestamp − pendingAt) / 60_000 restricted to losing trades (pnlPercentage < 0). UNITS: minutes. Null when there are no losing trades.*`,
      `*Avg Consecutive Win PNL: a "win streak" is a run of consecutive trades with pnlPercentage > 0 bounded by either a losing or break-even trade; this metric sums pnlPercentage within each streak and then averages those sums. UNITS: percent per streak. Higher = winning streaks are typically bigger. Null when there is not a single complete win streak in the stored history.*`,
      `*Avg Consecutive Loss PNL: a "loss streak" is a run of consecutive trades with pnlPercentage < 0 bounded by either a winning or break-even trade; this metric sums pnlPercentage within each streak and then averages those sums. UNITS: percent per streak (negative). Closer to 0 = losing streaks are typically smaller. Null when there is not a single complete loss streak.*`,
