...
class HeatmapStorage {
  ...
  public async getReport(
    ...
    return [
      ...
      `*Trades Per Year (portfolio = portfolioTradesPerYear): pooledN × 365 / poolSpanDays, where poolSpanDays = (latest closeTimestamp − earliest pendingAt across the whole pool) / day. UNITS: trades / year. Null when pooled span < ${MIN_CALENDAR_SPAN_DAYS} days OR raw value > ${MAX_TRADES_PER_YEAR} (sample too clustered to extrapolate reliably).*`,
      `*Total Trades (portfolio = portfolioTotalTrades): sum of per-symbol totalTrades over all tracked symbols. UNITS: integer count.*`,
      `*Avg Peak PNL (portfolio): trade-count-weighted mean of per-symbol avgPeakPnl over symbols whose avgPeakPnl is non-null. Weights are per-symbol totalTrades; symbols without any peak-snapshot signals don't contribute and don't dilute. UNITS: percent. Describes the portfolio's typical best-case unrealised excursion.*`,
      `*Avg Max Drawdown PNL (portfolio): trade-count-weighted mean of per-symbol avgFallPnl over symbols whose avgFallPnl is non-null. UNITS: percent (negative for losing excursions). Closer to 0 is better — describes the portfolio's typical worst-case unrealised excursion.*`,
      `*Peak Profit PNL (portfolio = portfolioPeakProfitPnl): MAX of per-symbol peakProfitPnl over all symbols whose value is non-null. UNITS: percent. The single best best-case excursion observed anywhere across the portfolio (tail behaviour the average hides).*`,
      `*Max Drawdown PNL (portfolio = portfolioMaxDrawdownPnl): MIN of per-symbol maxDrawdownPnl over all symbols whose value is non-null. UNITS: percent (negative). The single deepest unrealised dip observed anywhere across the portfolio.*`,
      `*Median PNL (portfolio = portfolioMedianPnl): median of pnlPercentage over the POOLED set of every closed signal of every symbol. UNITS: percent. Robust to outliers; compare with Pooled Sharpe's underlying portfolioAvg to detect skew (a large gap means one or two trades drag the arithmetic mean).*`,
      `*Avg Duration (portfolio = portfolioAvgDuration): arithmetic mean of (closeTimestamp − pendingAt) / 60_000 over the pooled set of every closed signal of every symbol that has both timestamps. UNITS: minutes.*`,
      `*Avg Win Duration (portfolio): mean hold time in minutes restricted to pooled winning closed signals (pnlPercentage > 0). UNITS: minutes. Null when there are no winning trades across the pool.*`,
      `*Avg Loss Duration (portfolio): mean hold time in minutes restricted to pooled losing closed signals (pnlPercentage < 0). UNITS: minutes. Null when there are no losing trades across the pool. Pair with Avg Win Duration to detect the asymmetry "winner-shorter-than-loser" (a red flag: cut winners short, let losers run).*`,
      `*Avg Consecutive Win PNL (portfolio): trade-count-weighted mean of per-symbol avgConsecutiveWinPnl over symbols whose value is non-null. A per-symbol "win streak" is a run of consecutive same-symbol closed signals with pnlPercentage > 0 bounded by a losing or break-even trade; the per-symbol metric averages the SUM of pnlPercentage within each streak. The portfolio aggregate weights by per-symbol totalTrades — concatenating streaks ACROSS symbols would be meaningless (different markets, different timeframes). UNITS: percent per streak.*`,
      `*Avg Consecutive Loss PNL (portfolio): trade-count-weighted mean of per-symbol avgConsecutiveLossPnl over symbols whose value is non-null. Same weighting rationale as above. UNITS: percent per streak (negative). Closer to 0 = losing streaks are typically smaller.*`,
      ...
      `*Expectancy (portfolio): (pooledWinCount / pooledN) × pooledAvgWin + (pooledLossCount / pooledN) × pooledAvgLoss over the pool. Break-even closed signals contribute 0. UNITS: percent per trade. Positive = profitable on average per trade across the portfolio. Null when pooled N < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      ``,
      `*--- Per-symbol table columns ---*`,
      `*Win Rate (column): per-symbol winCount / (winCount + lossCount) × 100. UNITS: percent in [0, 100]. Statistically noisy below 30 per-symbol signals; stable above 200.*`,
      ...
      `*Sortino Ratio (column): per-symbol avgPnl / downsideDeviation, where downsideDeviation = √( Σ min(0, pnlPercentage)² / N_total ) (canonical: MAR = 0, divide by N_total). UNITS: dimensionless. Null when that symbol's N < ${MIN_SIGNALS_FOR_RATIOS} OR no losing trades.*`,
      ...
      `*Expectancy (column): per-symbol (winCount / totalTrades) × avgWin + (lossCount / totalTrades) × avgLoss. Break-even trades contribute 0. UNITS: percent per trade.*`,
      ...
      `*Avg Peak PNL / Avg Max Drawdown PNL (columns): per-symbol arithmetic means of peakProfit.pnlPercentage / maxDrawdown.pnlPercentage over signals that carry that snapshot (signals without it are excluded — no zero dilution). UNITS: percent.*`,
      `*Peak Profit PNL / Max Drawdown PNL (columns): per-symbol MAX of peakProfit.pnlPercentage / MIN of maxDrawdown.pnlPercentage across the symbol's stored closed signals. UNITS: percent. The single best best-case and worst worst-case excursions for that symbol — tail behaviour the averages hide.*`,
      `*Avg Duration / Avg Win Duration / Avg Loss Duration (columns): per-symbol mean hold time in minutes — (closeTimestamp − pendingAt) / 60_000 — over all signals / winning signals / losing signals respectively. UNITS: minutes. Pair Avg Win vs Avg Loss durations to detect "let winners run, cut losers short" (Avg Win Duration > Avg Loss Duration is healthy).*`,
      `*Avg Consecutive Win PNL / Avg Consecutive Loss PNL (columns): a per-symbol "streak" is a run of consecutive same-symbol closed signals with same-sign pnlPercentage bounded by an opposite-sign or break-even trade; each metric sums pnlPercentage within each streak and averages those sums. UNITS: percent per streak. Pair with maxWinStreak / maxLossStreak to compare typical vs worst-case streak magnitude.*`,
